...
2. **Run L harness is BUILT + CODEX-BLESSED + smoke-GREEN** (2026-07-05). Design `RUNL_DESIGN_SPEC.md` (4 Codex
   design rounds) → implementation `lsr_runL_{fixture,verify,digest}.mjs` + `lsr_runL.mjs` + `lsr_runL_cases.json`
   (5 Codex impl rounds; brief `RUNL_IMPL_REVIEW.md`). Codex verdict: "Clean review. GO." Fixture + `--pre`
   smoke PASSED (all 4 joined; --pre VALID: L1 strict-fresh, L2 counterfactual winnerNwei=159 studyDay=2, full
   sha256 digest). **4-phase bound pipeline:** `lsr_preflight` (read-only) → `lsr_runL_fixture <runId>` (UI
   joins) → `lsr_runL_verify --pre <runId>` (read-only) → `lsr_runL <runId>` (measured, AI-graded) →
   `lsr_runL_verify --post <runId>` (read-only, FINAL verdict). Requires env `LSR_BUILD_ID` (owner-supplied
   deployed build/commit; bound across all artifacts) + `LSR_AUDIT_PW`. Clean pool = 9 (lsr_s28-36 fresh;
   smoke consumed s28/29/30). Core = L1-T/L1-M/L1-R (Day-1 completion, legacy smokes) + L2 (cross-class
   anchor NON-promotion — the sole flag discriminator). Certifies FLAG-OFF equivalence only.
