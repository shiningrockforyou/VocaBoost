...
3. **Gate isolation**: shadow class ids → `rehearsalClassIds` (with 25WT) **ONLY AFTER battery A (B3-on-shadow) completes [r65p — battery A requires B3's exclusive dark-window custody; the stamping carve-out (15_ §7) begins at registration]**; the real-class assertion (no real id ever in that list) runs before AND after; the flip choreography's "list empty" assertion is unchanged. **WINDOWS NEVER SPAN THE FLIP [r65p]: the flip choreography asserts NO `shadow_registry/window` doc exists (a stale window would quarantine stamp-less REAL rows out of the R2-18 abort evaluation during the soak).**
...
11. **Metrics tagging [v4]:** the dark build's ops_metrics writers stamp
   ...
   deleted at cleanup. **DRILL-E LAW [r66 — Codex r65 A4: removing the class from `rehearsalClassIds` while
the global marker is absent would disable the very label writers the OFF drill certifies]: drill E toggles
the ASSIGNMENT gate OFF only — the shadow class REMAINS registered in `rehearsalClassIds` throughout (writer
eligibility stays on; the R2-32 OFF-law posture is what the drill observes). Rehearsal-membership REMOVAL is
a separate teardown step with a DRAIN law: no removal while any composed-but-unsubmitted presentation is
outstanding (the compose-time snapshot governs through completion per contract (7)); the teardown asserts
drain-complete first.** **GENERATION/CACHE LAW [r62]:** `shadow_registry/0` carries a `generation` counter bumped by every registry write; server instances cache the uid-set keyed by generation with a ≤60s re-read; staleness ≤TTL is safe BY SCHEDULE — membership changes only OUTSIDE run windows (the driver writes the registry, waits >TTL, THEN starts the batteries; teardown reverses. **GENERATION-BOUND CLASSIFICATION [r63 — supersedes the r62p probe: a load-balanced callable reaches ONE instance per request and can NEVER prove what every warm instance holds]: safety does not depend on fleet-wide cache proof. Every ops_metrics writer stamps `registryGeneration` (the generation its cached set came from) on every row; during an audit window — THE WINDOW ARTIFACT [r64]: an Admin-written `shadow_registry/window` doc `{generation, startedAt, runId}` (created at battery start, deleted at teardown; its existence DEFINES 'an audit window' and its `generation` IS 'the registered generation') — every ops_metrics consumer (BOTH evaluators AND the §2.5 baseline computations) QUARANTINES rows whose `registryGeneration` is MISSING, non-integer, < the window generation, OR > it [r64 — deploy-skew rows without the stamp must not escape; `undefined < G` is false]; quarantined rows are indeterminate — never production-classified, never alert-feeding, never baseline-feeding. **QUARANTINE IS ITSELF A PUBLISHED SIGNAL [r64 — a stamping bug must not blackhole production monitoring silently]: the evaluator emits `quarantinedRowCount` per window to ops_metrics with its own alert threshold (a spike = investigate the writers immediately; the R2-18 abort evaluation is running BLIND on whatever is quarantined).** The audit evaluator consumes only window-generation `shadow === true` rows. A stale instance therefore cannot emit a production-classified shadow metric BY CONSTRUCTION. `getShadowRegistryGeneration` survives as a diagnostic only (no proof burden). STALE-CACHE INJECTION TESTS: plant rows stamped G−1, G+1, and UNSTAMPED during a G window ⇒ all three land in quarantine, all consumers, and the quarantine counter reflects them**). **REDUCED-SET SCOPE INPUT [r62]:** any battery running on a subset consumes a GENERATED shadow-subset class allowlist `audit/deepfix/shadow/scope-<runId>.json` through the EXISTING `--classAllowlist` surface of B1/B3/B4 — no new flags, same A6 binding. **UID GRANULARITY RULE [r62p, RESOLVED r64 — the partition-per-class reading contradicted §1's strict 1:1 mirror (dual-enrollment + cross-class strata make partition-aligned whole classes unsatisfiable)]: shadow classes stay STRICT 1:1 MIRRORS of the 33 (docId/assignments/composites per §1); each shadow class's `studentIds` = the reduced uid set ∩ that class's real membership (flagged-skip uids pruned AT CLONE TIME). PARTITIONS (C/D/E/F/G/J/K/L, §2b) are DRIVER-SIDE ACCOUNT LISTS in the frozen partition manifest — battery targeting, never class structure. `scope-<runId>.json` class allowlists exist only for whole-class B1/B3/B4 scoping; student-granular batteries consume the partition manifest directly.** The rules artifact gains the
...
class's `studentIds` = the reduced set ∩ its mirror class's real membership (§2.11 UID GRANULARITY RULE,
resolved r64; partitions are driver-side lists, never class structure). **CONSUMPTION [ONE sentence, r65 —
Codex r64 B1]: B1/B3/B4 whole-class operations consume generated class allowlists (each selected class's
membership = reducedSet ∩ realMirrorMembership); student-granular batteries consume the FROZEN account
partition manifest directly and assert every selected uid belongs to the mirrored class(es) it targets —
no battery equates a class with a partition.** Nothing
reads '947' as a constant. PRE-RUN GATE [reworded r65p — §2.11's resolution made partition-equality false]: the generated-manifest test asserts every selected shadow class's `studentIds` = reducedSet ∩ that class's real mirror membership, and that every partition-manifest uid belongs to the mirrored class(es) it targets, BEFORE any backfill/battery runs.** **CONTAINMENT TRIAD [v4 — honest scoping, r59-C6]: (1) the in-process guard covers CLONE/DRIVER writes
...
IMPLEMENTED — b1-expected-labels v6b [r59]; scope via classes, ONE law [r63])**. **THE COMPARISON LAW
[REWRITTEN r64 — `challengeDigest` hashes attempt docIds + classIds in its preimage; ids cannot be mapped
inside a hash, so the old mapped-digest byte-diff failed ~100% spuriously]: compare PER-UID under the uid
map — `words` (id-free `listId|wordId` keys; lists are shared un-remapped) + the per-student `digest` (hash
of words only) + `epochByList`; `challengeDigest` and `challengedAttemptIds` are EXCLUDED from the gate
(id-bearing preimages do not survive remapping). The REAL side is FILTERED to the run's reduced uid-set
before comparison (947−k rows, manifest-recorded). ROSTER-DRIFT RECONCILIATION: uids present live at clone
time but absent from the original baseline (joiners) are LISTED + EXCLUDED with published counts, never
silent mismatches. THE ATTEMPTS FIELD ALLOWLIST IS PINNED ⊇ THE REPLAY READ SET: studentId · classId ·
listId · sessionType/type · submittedAt · graded · manualOverride · teacherEdited · preOverride.score ·
score · scorePercent · totalQuestions · dayNumber · studyDay · answers[wordId, isCorrect, challengeStatus,
challengeReviewedAt] — one omitted field shifts exclusions and fails the gate.** → **audit A (needs ONLY Admin SDK — runs BEFORE/parallel with the dark
