...
4. **Kill the black boxes**: ship the status-messaging register (§12.3, 15 rows) so every hold/refusal/carry renders a
   bilingual reason + next step — targeting the real CS ticket classes (SUPPORT_RUNBOOK).
...
- **NOT STARTED (deepfix1 remainder — absorbed by this program):** D4/P5 ⚠️ one-way canonical migration · D5/P6 rules
  ...
  E1-E4 backlog (E2 session-lifecycle machine and E1 day-dispatch are absorbed by the container; E4 free-nav is this
  program's Wave 5; E3 policy-resolution folds into the lever work).
...
| Ship model | ONE release train; staged ACTIVATION (byte-identical container → dark free branch → per-class enablement) | 2026-07-25 (§12.1) |
...
| Review-pass gate | build (default-OFF lever `reviewPassThreshold`) — 2 open sub-decisions at build time (throttle-day y/n; retake-surface N) | banked 07-19, activated here |
...
- `MASTER_TASK_LIST.md` remains the **deepfix1 record** (everything through D3/D3.5 + the banked items' history). It gains
  ...
  **their gates travel with them unchanged**.
...
**Round 1 RAN 2026-07-25/26** (Codex r41 + 5 internal) — verdicts + fold map in task list §6; v2 of these docs is the fold.
**Round 2 (re-converge on v2) is REQUIRED before Wave-1 build authorization.**
