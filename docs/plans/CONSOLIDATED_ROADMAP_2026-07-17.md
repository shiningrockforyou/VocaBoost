...
- **PR-2** (engagement/`answeredCount` stamp + server I6 clamp + the OC-1/2/3/4 foundation mirrors: V2 pairing into `foundation.js:546-581`, engagement+grandfather into `:615/:638`, `review_recorded` hold-shape, **the ABSENT server lap-reset mirror**) — **direction-INDEPENDENT**; a **functions redeploy** folding these into the ALREADY-DEPLOYED P3 artifact (not a first deploy). Must precede the P4 flip (else the cutover re-mints I4/reverts hold-csd).
...
2. **P3 activation** — flip the 5 server flags incl. **`ANCHOR_VALIDATION_SHADOW=true`** (`--only functions`; folds in PR-2's mirrors). **Starts the ≥14-day M4 shadow clock** that gates P6.
...
**Reconciliation with the prior roadmap's RC corrections (still valid):** OC-1 (hold-csd server shape before P4), OC-2 (V2 pairing mirrored before P4), OC-3 (engagement+grandfather in foundation before P4), OC-4 (the ABSENT server lap-reset mirror), OC-5 (re-entry csd source), OC-6 (PR-2 rides the functions artifact), OC-7 (CS-toolchain retarget at P5), the recent-window `8→12` at all 4 sites (`progressService.js:229/423/441/449`), and the retake-rewind 5th csd writer (`MCQTest.jsx:960`) — all fold into Track-1/Track-2 as above.
