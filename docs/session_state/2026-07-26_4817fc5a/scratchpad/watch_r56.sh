#!/usr/bin/env bash
# Watch WinClaude's r56 Wave-C drive: fresh-read the win baton for handback (rev>=112 & turnOwner=claude),
...
SNAP=/tmp/claude-1000/-app/4817fc5a-d68b-443f-96c2-c94ed4b10bf5/scratchpad/r56_live_snapshot.txt
...
for i in $(seq 1 $MAX); do
  ...
  {
    echo "=== r56 watch tick $i/$MAX  $(date -u +%H:%M:%S)Z  baton: rev=$REV owner=$OWNER exec=$EXEC ==="
    if ls "$STEPS"/r56-*.jsonl >/dev/null 2>&1; then
      for f in "$STEPS"/r56-*.jsonl; do
        echo "--- $(basename "$f")  ($(wc -l < "$f") steps) ---"
        tail -n 2 "$f"
      done
    else
      echo "(no r56 step files yet — WinClaude not started emitting)"
    fi
  } > "$SNAP"
  if [ -n "$REV" ] && [ "$REV" -ge 112 ] && [ "$OWNER" = "claude" ]; then
    echo "HANDBACK DETECTED at tick $i: rev=$REV owner=$OWNER exec=$EXEC"
    echo "--- review file (if present) ---"
    [ -f /app/docs/plans/loop/win/reviews/winclaude_056.md ] && head -c 4000 /app/docs/plans/loop/win/reviews/winclaude_056.md
    exit 0
  fi
  sleep 20
done
