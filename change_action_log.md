...
| Date | File | Change |
|------|------|--------|
| 2026-06-29 | functions/index.js | **ROLLBACK: `GRADE_TOKEN_ENFORCED` true → false** (line 45, per owner request). Server-authoritative typed-grade enforcement is now OFF — submitVocabAttempt no longer rejects typed writes lacking a valid HMAC `gradeToken` and stamps no trusted marker (pre-G2 behavior for all cohorts incl. 26SM). `GRADE_TOKEN_MINT` left ON (additive/harmless: tokens still mint, client ignores). Updated the now-inaccurate inline warning comment to describe the OFF/rollback state. Code loads clean (`node -e require`). NOTE: deploy attempted but BLOCKED — service account `firebase-adminsdk-fbsvc@vocaboost-879c2` lacks `iam.serviceAccounts.ActAs` on runtime SA `vocaboost-879c2@appspot.gserviceaccount.com` (needs Service Account User role); not yet live in prod. |
