...
**DIAGNOSED 2026-08-04 (read-only code trace) — ROOT CAUSE = PROMPT/MODEL LENIENCY, NOT a code fail-open.** Every error path in `gradeTypedTest` (`functions/index.js:1059-1614`) is fail-CLOSED: empty response / JSON-parse failure / AI-omits-a-word / SDK error all THROW or default `isCorrect:false`, and post-grade validation only ever downgrades `true→false` (`:1543-1584`). So the 20/20 came from the AI's own output, not a fallback. The grader is **Anthropic Claude Haiku** (`claude-haiku-4-5-20251001`, `@anthropic-ai/sdk` — NOT OpenAI; CLAUDE.md corrected). Its rubric self-describes as "lenient / **Default to CORRECT**" (`:1356-1359`) with an 11:4 correct:wrong example ratio, and the one rule that should reject unrelated filler (rule 3, `:1363`) is stated in prose but **never reinforced by a worked example and never tested** — `scripts/grader-regression.mjs` has ZERO irrelevant-filler negative case; every prompt-tuning round (e.g. commit `0992f5f`) targeted the OPPOSITE failure (false-rejecting good Korean answers).

**⚠ AFFECTS THE REDESIGN TOO, not just legacy:** the engine typed leg uses the SAME grader — `functions/reviewV2/typedGrading.js:144` `defaultGrade` → `gradeTypedTest.run(...)`. So the leniency undermines the 92% TYPED GATE the whole redesign rests on, not only today's legacy path.

**CONFIRMED TYPED-SPECIFIC (win order 102, 2026-08-04):** an MCQ review test scored **32% (8/25)** on arbitrary choices — MCQ grading DISCRIMINATES. So the garbage-100% anomaly is specific to the TYPED (Haiku) grader, not grading in general — consistent with the prompt-leniency diagnosis. A deliberate wrong-but-plausible TYPED probe would still characterize how lenient the typed grader is (it's the only unquantified piece; the code root cause is settled).

**FIX (low-risk direction, but a LIVE-PATH grader change + a functions deploy ⇒ DAVID SIGN-OFF):** (1) add 1-2 explicit negative examples of generic filler ("answer"/"test"/"idk" → WRONG under rule 3) to the prompt, mirroring how rules 1/2/4 each carry an example; (2) add that as an `expect:false` row in `grader-regression.mjs`; (3) defense-in-depth code heuristic — flag/auto-fail when the IDENTICAL non-trivial response fills most/all rows of one test (an AI-independent signal no genuine studying produces; extends the blank/self-ref heuristics at `:168-190`,`:1543-1584`). **Risk to manage:** don't over-tighten into false-rejects (the failure the prompt was tuned away from) — the regression harness must still pass its positive cases after the change.
...
**FIX (per assessment; David directed "fix the prompt"):** (1) prompt: state that `student` is ALWAYS the literal typed text, never a placeholder + ONE worked WRONG example of the "answer"-as-filler shape (rule 3 finally gets its example); touch NOTHING else — the false-reject history (0992f5f) says the Korean-acceptance framing must stay byte-identical. (2) regression harness: add BATCH-mode cases (the suite graded singles only — it could NEVER have caught this): 20×/10×"answer" expect-all-wrong, 20×"answer1", and a 20×-genuine-correct batch to prove no batch over-tightening. (3) defense-in-depth code heuristic: pre-AI auto-fail identical non-blank responses spanning ≥8 rows — **ABSOLUTE threshold, the ≥80%-of-test leg was DROPPED at fix time** (the model flips at ~10 identical rows regardless of test fraction; 8-of-30 is exactly what a percentage leg would miss). Deploy rides `functions-deploy-engine` (David executes).

**FIX-TIME FINDING (2026-08-05, implementer case f): the exploit class is BROADER than identical rows — 20× VARIED filler ("answer1".."answer20") also scored 20/20 on the old prompt.** The heuristic (identical-strings by design) does NOT catch that class; the PROMPT fix alone defends it, with harness batch case (f) as the standing regression guard. Single-layered defense on that class — noted, accepted (carded E2 in the fold ledger).

**INDEPENDENT AUDIT 2026-08-05 (opus, full re-execution): CONDITIONAL GO-to-commit — all three legs CONFIRMED (add-only prompt diff reproduced opcode-level; 15/15 harness cases + 6/6+2/2 heuristic fixtures in the AUDITOR'S OWN runs; engine-leg compatibility verified) — but SEV-1 REFUTED the "no over-tightening" claim: hard-naming filler strings false-rejects the rare words they genuinely define (rejoinder←"answer", ordeal←"test", 3/3 stable; blast radius = responses literally equal to a named string, 19/20 rows in a mixed batch unaffected).** Adjudicated 2026-08-05: SOFTEN (example-conditioned filler rule + new harness case g guarding filler-as-genuine-definition), 2-iteration cap, fallback = explicit acceptance. **Production collision scan (read-only, all 46 lists / 66,141 words): ZERO definitions begin with answer/reply/test/trial; 335 mid-definition mentions only (e.g. ordeal "…a severe test") — the false-reject class is real but marginal in live data.** SEV-4 carded (early-exit results-array order is not presentation order — pre-existing class, cosmetic, cheating-shape only).
