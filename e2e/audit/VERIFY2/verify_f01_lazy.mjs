/**
 * VERIFY2 — F01 Pool Collapse Fix Verification (lazy/TOP persona)
 *
 * OBJECTIVE: Walk ~6-8 review sessions forward from lazy's current Day 18.
 * For each review session, capture served words (by wordId) + pre-session
 * study_state (status + returnAt). Count served words where preStatus=MASTERED
 * with future returnAt. EXPECT 0 every day.
 *
 * Identity-based check (not position-based — prior position checks gave false positives).
 */
...
const LAZY_EMAIL = 'audit_lazy_01_top@vocaboost.test'
const LAZY_PASSWORD = 'AuditPass2026!'
const LAZY_UID = 'VBgBmlrlzXVPzURmABkdDBGtKd42'
...
const STATUS_PATH = join(AGENT_LOGS_DIR, 'VERIFY2.status.json')
...
function log(ev) {
  ...
  console.log('[VERIFY2]', JSON.stringify(ev).substring(0, 250))
}
...
const readStudyStateByWordId = async (uid, wordId) => {
  const s = await initAdmin().doc(`users/${uid}/study_states/${wordId}`).get()
...
// ── Word cache (wordId → position/definition) ──
...
async function navSession(page) {
  ...
  for (const name of ["Start Today's Session", 'Start Session', 'Start Today', 'Begin Session']) {
    ...
    if (await btn.isVisible({ timeout: 2000 }).catch(() => false)) {
      log({ type: 'nav_session_button', label: name })
      await btn.click()
      await wait(2500)
      return true
    }
...
async function h2Guard(page, dayNum) {
  ...
  if (/resume.*day\s*\d|move.*on.*next\s*day|retry.*review.*test/i.test(bodyText)) {
    log({ type: 'h2_reentry_modal', day: dayNum })
...
// ── Handle new-word typed test (lazy: type gibberish or simple answer) ──
async function handleNewWordTest(page, dayNum) {
  ...
  log({ type: 'new_word_test_inputs', day: dayNum, count })

  // For lazy, just type something to proceed
  ...
  if (await subBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
    await subBtn.click()
    log({ type: 'new_test_submitted', day: dayNum })
    await wait(25000) // wait for AI grading
  } else {
    await wait(25000)
  }
}

// ── Capture review test words with identity ──
// Returns array of { wordText, wordId, position, preStatus, preReturnAtMs }
async function captureAndRunReviewTest(page, dayNum, preStudyStates, nowMs) {
  const servedWords = []
  const maxQ = 80
  log({ type: 'review_test_start', day: dayNum })

  // Build lookup map from study_states
  const stateByWordId = {}
  for (const ss of preStudyStates) {
    stateByWordId[ss.id] = ss
  }
  ...
  for (let q = 0; q < maxQ; q++) {
    ...
    if (/test complete|test results|all done|finished|\d+%\s*(correct|score)/i.test(bodySnip)) {
      log({ type: 'review_complete', day: dayNum, q })
      break
    }
    ...
    const wordText = await page.evaluate(() => {
      ...
      // Also try to find a prominent word display
      const wordEls = [...document.querySelectorAll('[class*="word"], [class*="question"], strong, b')]
    ...
    if (wordText) {
      const entry = { wordText, wordId, position, preStatus, preReturnAtMs, questionIdx: q }
      servedWords.push(entry)

      // Flag MASTERED with future returnAt
      const isMasteredLeak = preStatus === 'MASTERED' && (preReturnAtMs == null || preReturnAtMs > nowMs)
      if (isMasteredLeak) {
        log({ type: 'MASTERED_LEAK_DETECTED', day: dayNum, wordText, wordId, position, preStatus, preReturnAtMs })
      }
    }

    // Answer the question (click any option)
...
// ── Navigate to review test after new word test ──
async function navToReview(page, dayNum) {
  ...
  // Re-enter session via client-side nav
  log({ type: 'nav_review_re_enter', day: dayNum })
...
// ── Main ──
async function main() {
  log({ type: 'verify2_f01_start', persona: 'lazy', class: 'TOP' })

  // Read initial state
  const initialCP = await readCP(LAZY_UID)
  ...
  console.log(`=== VERIFY2 F01 | Lazy/TOP | CSD=${initialCSD}, TWI=${initialTWI} ===`)

  // Session anchor: start from 2026-07-07 (far future, clear of real today)
  // CSD=18, so next session is Day 19. Walk 6 sessions.
  const ANCHOR_DATE = new Date('2026-07-07T09:00:00+09:00')
  ...
  try {
    for (let i = 0; i < NUM_SESSIONS; i++) {
      ...
      console.log(`\n════ Day ${dayNum} | Fake: ${sessionDate.toISOString().substring(0, 10)} ════`)
      log({ type: 'day_start', day: dayNum, fakeDate: sessionDate.toISOString() })

      // Pre-session: read ALL study states
      const preStudyStates = await readAllStudyStates(LAZY_UID)
      const stateByWordId = {}
      for (const ss of preStudyStates) {
        stateByWordId[ss.id] = ss
      }
      ...
      log({ type: 'pre_state', day: dayNum, totalStates: preStudyStates.length, mastered: preMastered.length, masteredFuture: preMasteredFuture.length })

      // ── Create fresh browser context with date shim ──
      ...
      try {
        await login(page, LAZY_EMAIL, LAZY_PASSWORD)

        // Verify shim
        const shimCheck = await page.evaluate(() => ({
          now: Date.now(),
          fakeNow: window.__VERIFY2_FAKE_NOW_MS
        })).catch(() => null)
        log({ type: 'shim_check', day: dayNum, shimCheck })

        // Navigate to session
        const navOk = await navSession(page)
        if (!navOk) {
          sessionBlocked = true
          blockReason = 'Could not navigate to session'
          log({ type: 'session_blocked', day: dayNum, reason: blockReason })
        } else {
          await wait(2000)
          ...
          while (h2Attempts < 3) {
            ...
            if (h2Attempts >= 3) {
              sessionBlocked = true
              blockReason = 'H2 stale unrecoverable'
              break
            }
          ...
          if (!sessionBlocked) {
            // Skip to test (bypasses flashcard study)
            await skipToTest(page)
            await wait(2000)
            ...
            log({ type: 'page_state', day: dayNum, hasInput, bodySnip: bodyText.substring(0, 100) })

            if (hasInput) {
              // Handle new-word test first
              ...
              if (reviewNavOk) {
                ...
                sessionBlocked = true
                blockReason = 'Could not navigate to review test'
                log({ type: 'review_nav_failed', day: dayNum })
              }
            } else {
              // Already on review test (day 2+ re-entry)
              servedWords = await captureAndRunReviewTest(page, dayNum, preStudyStates, nowMs)
            }
          }
        }
        ...
        log({ type: 'session_exception', day: dayNum, error: err.message })
      } finally {
        await page.close().catch(() => {})
        await context.close().catch(() => {})
      }

      // ── Analyze served words for MASTERED leaks ──
      ...
      const dayResult = {
        ...
        fakeNowMs: nowMs,
        preState: { total: preStudyStates.length, mastered: preMastered.length, masteredFuture: preMasteredFuture.length },
        ...
        masteredLeaks: masteredLeaks.map(w => ({ wordId: w.wordId, position: w.position, preStatus: w.preStatus, preReturnAtMs: w.preReturnAtMs })),
      ...
      // Write evidence
      writeFileSync(join(EVIDENCE_DIR, `f01_lazy_day${dayNum}.json`), JSON.stringify(dayResult, null, 2))
      log({ type: 'day_result', day: dayNum, served: servedWords.length, masteredLeaks: masteredLeaks.length, blocked: sessionBlocked })

      console.log(`  Day ${dayNum}: served=${servedWords.length}, MASTERED_LEAKS=${masteredLeaks.length}, blocked=${sessionBlocked}`)
      if (masteredLeaks.length > 0) {
        console.log(`  *** F01 FIX FAILED: ${masteredLeaks.length} MASTERED words served! ***`)
        masteredLeaks.forEach(w => console.log(`    - wordId=${w.wordId} pos=${w.position} returnAt=${w.preReturnAtMs}`))
      }

      // Advance date for next session
      sessionDate = nextWeekday(sessionDate)
      await wait(3000) // Let Firestore settle
    }
  } finally {
  ...
  console.log('\n═══ VERIFY2 F01 SUMMARY (lazy/TOP) ═══')
  console.log(`Sessions run: ${results.length}`)
  console.log(`Total MASTERED leaks: ${totalLeaks}`)
  console.log(`F01 FIXED IN PROD: ${f01Pass ? 'YES — PASS' : 'NO — FAIL'}`)

  const summary = {
    test: 'F01_POOL_COLLAPSE',
    persona: 'lazy/TOP',
    ...
    perDay: results.map(r => ({
      ...
      masteredLeaks: r.masteredLeakCount,
      blocked: r.blocked
    }))
  }
  writeFileSync(join(EVIDENCE_DIR, 'f01_lazy_summary.json'), JSON.stringify(summary, null, 2))
  writeFileSync(STATUS_PATH, JSON.stringify({ agent: 'VERIFY2', f01_lazy: summary, updatedAt: new Date().toISOString() }, null, 2))

  log({ type: 'verify2_f01_complete', totalLeaks, f01Pass })
  return summary
}

main().then(s => {
  console.log('\nDone:', JSON.stringify(s, null, 2))
  process.exit(0)
}).catch(err => {
