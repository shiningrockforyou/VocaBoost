...
- **runId:** `winclaude-selfcheck`  **target:** `baseline`
- **git:** `a967f54` (HEAD `a967f544e0f3d4bce72861ad82a34d8e2ec27206`) dirty=true (333 paths)
- **dist:** local dist/ built unknown (NOT the deployed bundle; freshness unverified)
- **flag files dirty:** M functions/index.js, M src/config/featureFlags.js, ?? functions/foundation.js
- **run:** 2026-07-14T13:40:52.105Z
...
| ❌ | DG-4 | DG-4 | commits∧fixStr∧hygiene | commits=false fixStr=true hygiene=true | **FAIL** |
| ❌ | DG-4b | DG-4 | fix>0 ∧ audit==0 | fixInDist=0 auditInDist=0 | **FAIL** |
...
| ❌ | CUT-1b | CUT-1 | present (dormant) | assetsWithClassProgress=0 | **FAIL** |
| ❌ | RET-1 | RET-1 | > 0 | 0 | **FAIL** |
...
- **DG-4** (FAIL): commits: src/services/studyService.js→NONE \| src/pages/DailySessionFlow.jsx→NONE \| src/pages/Dashboard.jsx→NONE. fix-unique "Please Reload" ×1 in TypedTest.jsx (full: "Couldn't Grade — Please Reload"). audit/scripts imports from src: none.
- **DG-4b** (FAIL): local dist/ built unknown (NOT the deployed bundle; freshness unverified). "Please Reload" in 0 dist asset(s); audit/scripts refs in 0.
...
- **CUT-1b** (FAIL): local dist/ built unknown (NOT the deployed bundle; freshness unverified). class_progress in 0 dist asset(s) — expected present in baseline (goes 0 only after P4 cutover + P7 retire).
- **RET-1** (FAIL): class_progress refs: 0 (src=0 functions=0 scripts/cs=0). Nothing retired in baseline.
